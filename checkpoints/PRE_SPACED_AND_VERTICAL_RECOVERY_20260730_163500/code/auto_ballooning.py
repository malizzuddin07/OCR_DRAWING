...
def run_rescue_measurements(image_path, existing_rows):
    image = cv2.imread(str(image_path))
    if image is None:
        return []
    ...
    ocr = create_paddle_ocr()
    raw_items = []
    raw_items.extend(rescue_vertical_decimal_dimensions(ocr, image, existing_items))
...
def extract_local_gdt_value_rows(image_path, symbol_detections, existing_rows):
    ...
    for symbol in symbol_detections:
        ...
        if symbol_class not in {"gdt_parallelism_symbol", "gdt_perpendicularity_symbol", "gdt_frame_symbol"}:
            continue
        if float(symbol.get("Confidence", 0) or 0) < 0.70:
            continue
...
def extract_local_symbol_context_rows(image_path, symbol_detections, existing_rows):
    ...
    for symbol in symbol_detections:
        ...
        if symbol_class not in target_classes:
            continue
        if float(symbol.get("Confidence", 0) or 0) < 0.62:
            continue
...
def detector_ocr_candidate_is_better(best, candidate):
    ...
    return bool(
        ...
        and candidate_confidence >= max(0.72, best_confidence - 0.08)
    )
...
def extract_yolo_text_box_rows(image_path, symbol_detections, existing_rows):
    ...
    for symbol in symbol_detections:
        ...
        if symbol_class not in target_classes:
            continue
        if float(symbol.get("Confidence", 0) or 0) < 0.58:
            continue
        ...
        if symbol_class == "dimension_text":
            ...
            padding_ratios.extend([0.20, 0.32])
        ...
        for padding_ratio in padding_ratios:
            ...
            for orientation, variant in variants:
                ...
                if re.fullmatch(r"\d", value or text):
                    score -= 0.40
                candidate = (score, recognition_confidence, orientation, measurement_type, value, text)
        ...
        _, recognition_confidence, orientation, measurement_type, value, text = best
        mapped_item = {
            "text": text,
            "confidence": min(
        ...
        row["Review Reason"] = "; ".join(
            reason for reason in [row.get("Review Reason", ""), f"YOLO {symbol_class} crop"] if reason
        )
...
def general_tolerance_for_specification(specification, nominal, general_tolerances=None):
    if general_tolerances:
        ...
        if not nominal_match or nominal_match.group(2) is None:
            return None
...
def parse_tolerance(specification, apply_general=True, general_tolerances=None):
    ...
    if plus_minus:
        ...
        if len(signed_deviations) >= 2:
            ...
            if explicit_minus:
                minus = float(explicit_minus.group(1))
...
def is_shadowed_by_detector_measurement(measurement, all_measurements):
    ...
    for other in all_measurements:
        ...
        if other_type not in {
            ...
            "metric_thread",
            "surface_finish",
        } and not detector_has_complete_tolerance:
            continue
...
def is_nested_reference_fragment(measurement, all_measurements):
    ...
    if str(measurement.get("Measurement Type", "")) != "plain_dimension":
        return False
    value = corrected_measurement_specification(measurement)
...
def are_same_dimension_candidate(row, existing):
    ...
    x_tolerance = 260 if hole_callout_pair else 70
    y_tolerance = 90 if hole_callout_pair else 45
...
def split_tolerance_match_score(nominal_row, tolerance_row):
    ...
    max_dy = min(220, max(120, nominal_height * 2.8, tolerance_height * 2.5))
    ...
    return dx * 1.2 + dy + stacked_bonus + fragment_penalty + distant_vertical_penalty
...
def group_split_tolerance_measurements(rows):
    ...
    return [row for row in rows if id(row) not in grouped_ids | shadowed_tolerance_ids]
...
def is_duplicate_hole_callout_noise(measurement, all_measurements):
    ...
    value = normalise_text(measurement.get("Extracted Value") or measurement.get("OCR Text") or "")
    if not has_non_ascii_text(value):
        return False
    ...
    for other in all_measurements:
        ...
        if numbers != other_numbers:
            continue
        if not trusted_hole_callout_text(other_value):
            continue
...
def is_report_excluded(measurement, image_shape=None):
    ...
    # Detector-guided OCR comes from a model-confirmed measurement box. Fixed
    # page zones vary between drawing templates and must not discard it.
    if is_detector_guided_row(measurement):
        return False
    ...
    return bool(exclusion_region_label(center_x, center_y, REPORT_EXCLUSION_REGIONS))
...
def assign_stable_balloon_numbers(rows):
    ...
    for row in rows:
        ...
        if not is_group_continuation:
            balloon_no += 1

        if multiplier_count > 1:
            row["Balloon No"] = f"{balloon_no}.{row['Multiplier Index']}"
            row["Display Balloon No"] = str(balloon_no)
        elif subrow_count > 1:
            row["Balloon No"] = f"{balloon_no}.{row['Subrow Index']}"
            row["Display Balloon No"] = str(balloon_no)
        else:
            row["Balloon No"] = str(balloon_no)
...
def normalize_compound_callout_rows(measurement_rows):
    ...
    for original in measurement_rows:
        ...
        drill_size_match = re.search(r"(?:(\d{1,2})\s*X\s*)?(\d+(?:\.\d+)?)\s*(?:キリ|DRILL)", text, re.IGNORECASE)
        if drill_size_match:
...
def tight_balloon_frame_box(row, image_width, image_height):
    ...
    compact_types = {"surface_finish"}
    if measurement_type in compact_types:
        text_width_cap = max(52, min(width, 36 + len(display_text) * 18))
        text_height_cap = max(34, min(height, 72))
        center_x = x + width / 2
        center_y = y + height / 2
        if width > text_width_cap:
            x = int(center_x - text_width_cap / 2)
            width = int(text_width_cap)
        if height > text_height_cap:
            y = int(center_y - text_height_cap / 2)
            height = int(text_height_cap)
...
def separate_mild_frame_overlaps(rows, frames, maximum_overlap_ratio=0.25):
    ...
    for first_index in range(len(adjusted)):
        for second_index in range(first_index + 1, len(adjusted)):
            ...
            if overlap_area / min(first_area, second_area) > maximum_overlap_ratio:
                continue
            try:
