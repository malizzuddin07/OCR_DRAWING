...
def rescue_vertical_decimal_dimensions(ocr, image, existing_items):
    image_height, image_width = image.shape[:2]
    rescued = []
    scale = 3

    for crop_info in find_vertical_decimal_text_crops(image):
        ...
        rotations = [
            ("cw", cv2.rotate(cleaned, cv2.ROTATE_90_CLOCKWISE), transform_rotated_cw_crop_box_to_original),
        ]
        for _, rotated, map_box in rotations:
            ...
            for merged in merge_decimal_ocr_fragments(scaled_items):
                mapped_box = map_box(
                    merged["box"],
                    crop_x1,
                    crop_y1,
...
def rescue_small_text_dimensions(ocr, image, existing_items):
    ...
    for x1, y1, x2, y2 in small_text_tile_regions(image_width, image_height):
        ...
        for item in local_items:
            ...
            if is_duplicate_item(mapped, existing_items + rescued):
                continue
            if is_near_existing_text(mapped, existing_items + rescued, max_distance=24) and len(mapped["text"]) <= 4:
                continue
            rescued.append(mapped)
...
def extract_measurement_ocr_items(ocr, image):
    ...
    items.extend(rescue_small_text_dimensions(ocr, image, items))
    items.extend(rescue_single_digit_dimensions(ocr, image, items))
    return clean_ocr_results(items, image_width=image_width, image_height=image_height)
