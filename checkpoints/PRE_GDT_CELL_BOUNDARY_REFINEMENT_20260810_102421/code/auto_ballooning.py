...
def recognize_gdt_datum_cell(recognizer, crop):
    ...
    if compact in {"X", "Y", "Z"}:
        return compact
    return {"2": "Z", "7": "Z", "V": "Y"}.get(compact, "")
    ...
    for first_index in range(len(adjusted)):
        for second_index in range(first_index + 1, len(adjusted)):
            ...
            if abs(first_center[0] - second_center[0]) >= abs(first_center[1] - second_center[1]):
                raw_overlap = max(0, min(first_raw[2], second_raw[2]) - max(first_raw[0], second_raw[0]))
                ...
                raw_overlap = max(0, min(first_raw[3], second_raw[3]) - max(first_raw[1], second_raw[1]))
                if overlap_ratio > maximum_overlap_ratio and (not raw_geometry_available or raw_overlap > 2):
                    continue
