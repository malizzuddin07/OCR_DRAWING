...
_Exported on 08/14/2026 at 08:17:52 GMT+8 from OpenAI Codex via WayLog_
...
- product-design:index: Use when Product Design is explicitly invoked, or when the user's main goal is to explore a design, research UX, audit or critique a flow, faithfully clone a visual source, check a built design, or share a prototype. Do not use Product Design for ordinary implementation unless the user explicitly asks for it. (file: r6/index/SKILL.md)
...
can you give me the workflow of this system from end to end process
...
I’ll trace the actual workflow from the repository and its generated artifacts, then explain it end-to-end in simple terms. I’ll keep this read-only and distinguish confirmed behavior from any inference.
...
The current system converts a single-page engineering drawing PDF into:

- A ballooned drawing
- An editable list of inspection characteristics
- An FA inspection Excel report
- A corrected ballooned PDF
- A final approved evidence package

```text
User starts website
        ↓
Uploads single-page PDF
        ↓
System creates a unique Job ID and job folder
        ↓
PDF is rendered as a high-resolution image
        ↓
Title block information is extracted
        ↓
Dimensions and engineering symbols are detected
        ↓
OCR results are cleaned, combined, and validated
        ↓
Inspection characteristics are generated
        ↓
Balloons are positioned on the drawing
        ↓
Initial PDF, PNG, Excel, and diagnostics are produced
        ↓
User reviews and corrects the results
        ↓
System performs quality checks
        ↓
User approves the completed drawing
        ↓
Final approved PDF, Excel, source PDF, crops,
approval record, and ZIP package are saved
```
...
This is the documented active web workflow in [CURRENT_WEB_WORKFLOW.md](C:/Users/izzuddin/Desktop/OCR%20DRAWING/docs/CURRENT_WEB_WORKFLOW.md).

---

### 2. Upload the engineering drawing

The user selects a PDF through the website.
...
1. Accepts only a `.pdf` file.
2. Generates a unique 12-character `job_id`.
3. Creates this folder:

```text
generated_jobs/jobs/<job_id>/
```

4. Saves the uploaded PDF inside:

```text
generated_jobs/jobs/<job_id>/upload/
```

5. Starts processing in the background.
6. Returns the Job ID to the browser.

The browser regularly polls:
...
This behaviour is implemented in [server.py](C:/Users/izzuddin/Desktop/OCR%20DRAWING/webapp/server.py:717).

Important limitation: the current processing function accepts only a single-page PDF. A multi-page PDF produces an error.

---

### 3. Render the source drawing

The system opens the PDF and renders the drawing into:

```text
original.png
```

This image becomes the common visual input for:

- Title-block OCR
- Measurement OCR
- Local YOLO detection
- Roboflow detection, when enabled
- Balloon placement
- Manual crop OCR
- Final corrected output

The main pipeline begins in [auto_ballooning.py](C:/Users/izzuddin/Desktop/OCR%20DRAWING/code/auto_ballooning.py:6245).

---

### 4. Extract drawing metadata

The system first tries to obtain metadata from:

1. The PDF filename or known drawing mapping
2. Title-block OCR for fields that are still missing
...
Title-block investigation images and information are stored under:

```text
generated_jobs/jobs/<job_id>/titleblock_debug/
```

The filename/known mapping takes priority, and title-block OCR fills missing information.

---

### 5. Run parallel discovery

Three independent operations run at approximately the same time:

```text
                    ┌─ Local YOLO symbol detection
Original drawing ───┼─ Roboflow detection, when enabled
                    └─ Embedded PDF text extraction
```

#### Local YOLO detection

The local detector looks for engineering-related regions or symbols.

#### Roboflow detection

If configured and enabled, the system also sends image data to the configured Roboflow Workflow.

Local and Roboflow detections are then:

1. Combined
2. Deduplicated
3. Filtered for implausible detections
4. Saved for diagnostics

Relevant output files include:

```text
symbol_detections.xlsx
roboflow_detections.xlsx
detector_diagnostics.json
```

Roboflow is an additional detector source; it is not the entire OCR pipeline.

---

### 6. Extract measurements and callouts

The system gathers potential characteristics from several sources:
...
Full-page OCR
        +
Degree and angle recovery
        +
Dimension-gap recovery
        +
OCR around YOLO text boxes
        +
OCR around engineering symbols
        +
Stacked-tolerance recovery
        +
Drill-callout recovery
        +
GD&T context recovery
```

For normal browser uploads, the wrapper currently requests a fresh run instead of reusing the legacy measurement cache.

This stage searches for items such as:
...
- Stacked tolerances
- Hole and drill callouts
- Surface-related callouts
- GD&T characteristics
- Datum-related information

The combined raw results are deduplicated and saved as:
...
### 7. Build inspection characteristics

The raw OCR candidates are converted into structured FA characteristics.

Conceptually, each row contains information such as:
...
Position on drawing
OCR source
Confidence or review information
```

The system also filters candidates that appear to be:

- Incomplete
- Duplicated
- Located in excluded areas
- Implausible engineering callouts
- Notes or unrelated drawing text

Rejected candidates are saved separately:

```text
rejected_candidates.xlsx
```

This is useful for investigating why something was not ballooned.

---

### 8. Generate the initial outputs

The accepted characteristics are numbered and balloons are drawn on the engineering drawing.

The first-pass outputs include:
...
`balloon_layout.json` records layout problems such as balloon overlaps. These can cause the job to finish as `needs_review` instead of `success`.

---

### 9. Display results for human review

The browser displays:

- The ballooned drawing
- Extracted drawing metadata
- The characteristics table
- Quality-check results
- Controls for correcting the output

At this point, the AI output is a draft. It is not automatically considered approved.

The user should compare every row and balloon against the original drawing.

---

### 10. Correct the results

The user can review and modify the characteristics table, including supported fields such as:
...
- Balloon size
- Balloon rotation
- Review status or related fields

When relevant edits are made, the frontend calls:

```text
POST /api/render-preview
```

The corrected preview is regenerated from the current table. Therefore, the edited table becomes the source of truth for the corrected output.

---

### 11. Add a missed characteristic manually

If OCR misses a dimension, the user can:

1. Activate the add-balloon function.
2. Draw a box around the missing dimension.
3. The selected crop is sent to:

```text
POST /api/ocr-crop
```

4. The backend runs OCR on that selected region.
5. A new editable row is returned.
6. The system adds a balloon and regenerates the preview.

The user identifies the location, while the system attempts to extract the symbol and value.

---

### 12. Export corrected working files

Before final approval, the user can generate working corrected files.
...
```text
fa_inspection_report_corrected.xlsx
```
...
#### Corrected ballooned PDF

```text
POST /api/export-corrected-pdf
```
...
These exports use the current corrected table and metadata, not merely the original OCR results.

---

### 13. Quality checks before approval

The user must confirm that the complete drawing has been checked.

The server then verifies that:

- At least one checked characteristic exists
- The original result snapshot exists
- The uploaded source PDF still exists
- `original.png` exists
- Required balloons are not missing
- Required golden-output comparisons pass when applicable
- Balloon overlap checks pass
- Other recorded quality checks are resolved

If these checks fail, approval is blocked with:

```text
status: needs_review
```

The user must fix the reported problems before trying again.

---

### 14. Final approval

After the complete drawing is reviewed, the frontend sends:
...
1. Generates the final corrected PDF.
2. Generates the final corrected Excel report.
3. Copies the original source PDF.
4. Records differences between original OCR and corrected rows.
5. Saves crop images for the approved characteristics.
6. Records detector/model diagnostics.
7. Creates an immutable approval record.
8. Creates a ZIP package.

The approved package contains approximately:

```text
approved_ballooned_drawing.pdf
approved_fa_inspection_report.xlsx
source_<original-filename>.pdf
approval.json
approved characteristic crops
```

Approval files are stored separately from temporary job outputs so there is a permanent, auditable result.

If the exact same source and corrected contents have already been approved, the system can return the existing approval rather than creating an unnecessary duplicate.

---

## What “learning” currently means

Approval captures valuable correction evidence, including:
...
However, the current approval record explicitly says:
...
```text
Approved correction → saved as future training evidence
```

It does not mean:

```text
Approved correction → model immediately retrains itself
```

This is safer because unverified automatic retraining could damage the model.

---

## Human and system responsibilities

| Stage | System responsibility | User responsibility |
|---|---|---|
| Upload | Validate and save PDF | Select correct single-page drawing |
| Detection | Find symbols and measurement regions | Check for missing/false detections |
| OCR | Read dimensions and callouts | Confirm every extracted value |
| Ballooning | Number and position balloons | Correct positions, sizes and rotations |
| FA report | Generate structured Excel rows | Verify values, tolerances and metadata |
| Quality gate | Detect known output problems | Resolve every reported problem |
| Approval | Save immutable evidence package | Confirm complete drawing was checked |
| Learning data | Save approved corrections and crops | Ensure only correct information is approved |

The most important operational rule is: **a completed OCR job is only a draft; human approval is the final authority.**

<oai-mem-citation>
<citation_entries>
MEMORY.md:97-99|note=[used prior architecture summary to orient current source verification]
MEMORY.md:289-294|note=[used prior runtime workflow notes to identify active upload and job artifact paths]
</citation_entries>
<rollout_ids>
019fd473-c26c-7202-92d2-f355b6ad2b73
019f49f7-7305-77d1-9b12-3a77a5c72ac5
</rollout_ids>
</oai-mem-citation>
