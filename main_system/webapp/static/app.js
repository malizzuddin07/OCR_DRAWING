...
function renderTable(dataArray) {
    ...
    dataArray.forEach((item, index) => {
        ...
        tr.innerHTML = `
            ...
            <td><button type="button" class="small-danger-btn" data-action="delete-row" data-index="${index}">Delete</button></td>
        `;
...
function createEmptyManualRow() {
    const nextNumber = String(currentExtractedData.length + 1);
    return {
...
async function addManualBalloonFromBox(box) {
    ...
    try {
        ...
        const row = data.row || createEmptyManualRow();
