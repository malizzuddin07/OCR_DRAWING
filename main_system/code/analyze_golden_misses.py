...
def group_by_box(rows):
    ...
    for row in rows or []:
        ...
        if subrow_count > 1:
            # Compound rows such as CBORE plus DEPTH are one callout even when
            # the detector gives each line a separate tight box. Group them by
            # their parent balloon and shared specification before comparing.
            balloon_root = clean(row.get("Balloon No", "")).split(".", 1)[0]
            specification = re.sub(r"\s+", "", clean(row.get("Specification", "")).upper())
            key = ("compound", balloon_root, specification)
        else:
            # Rounding removes harmless JSON float formatting differences.
            key = ("box",) + tuple(round(value, 2) for value in box)
        grouped.setdefault(key, []).append(row)

    result = []
    for values in grouped.values():
        boxes = [item_box(row) for row in values]
        x1 = min(box[0] for box in boxes)
        y1 = min(box[1] for box in boxes)
        x2 = max(box[0] + box[2] for box in boxes)
        y2 = max(box[1] + box[3] for box in boxes)
        result.append({"box": (x1, y1, x2 - x1, y2 - y1), "rows": values})
...
def signature_field_value(field, value):
    text = clean(value).upper()
    if field == "Dimension" and re.fullmatch(r"[+-]?\d+(?:\.\d+)?", text):
        # `86` and `86.0` are the same engineering value. Spreadsheet display
        # formatting must not create a false accuracy failure.
        try:
            return f"{float(text):.9g}"
        except ValueError:
            pass
    if field == "Measurement Type" and text == "MANUAL":
        # Old approved corrections used the storage label `manual`; an
        # automatically recovered plain dimension with identical fields is
        # semantically equivalent.
        return "PLAIN_DIMENSION"
    if field in {"Report Symbol", "Symbol", "Dimension"}:
        # Formatting spaces do not change an inspection characteristic.
        return re.sub(r"\s+", "", text)
    if field in {"Tolerance -", "Tolerance +", "MIN", "MAX"} and text:
        try:
            return f"{float(text):.9g}"
        except ValueError:
            pass
...
def row_signature(row):
    # ``Report Symbol`` is the final user-facing symbol.  Older approved rows
    # can also retain the same value in the internal ``Symbol`` field, while
    # freshly generated rows may leave that internal field blank.  Comparing
    # both fields therefore creates false mismatches for identical reports.
    report_symbol = clean(row.get("Report Symbol", ""))
    effective_symbol = report_symbol or clean(row.get("Symbol", ""))
    measurement_type = clean(row.get("Measurement Type", ""))
    if re.match(r"^DEPTH\s+\d", clean(row.get("Dimension", "")), re.IGNORECASE):
        # Older approved depth rows used `manual` or `plain_dimension`; the
        # production parser correctly represents the same item as a hole
        # callout.
        measurement_type = "hole_callout"
    return tuple(
        signature_field_value(
            field,
            measurement_type if field == "Measurement Type" else row.get(field, ""),
        )
        for field in (
            "Dimension",
            "Tolerance -",
    ...
    ) + (signature_field_value("Report Symbol", effective_symbol),)
...
def analyze_drawing(manifest, golden_root, candidate_root, output_root):
    ...
    candidate_dir = candidate_root / drawing
    if not (candidate_dir / "characteristics.json").is_file() and (
        candidate_root / "characteristics.json"
    ).is_file():
        candidate_dir = candidate_root
    current_path = candidate_dir / "characteristics.json"
    image_path = candidate_dir / "original.png"
...
def select_manifests(manifests, drawing=None):
    if drawing is None:
        if len(manifests) != 5:
    ...
    selected = [
        ...
        if clean(manifest.get("drawing_number", "")).upper() == clean(drawing).upper()
    ]
    if not selected:
        raise ValueError(f"Golden drawing was not found: {drawing}")
    return selected


def run_analysis(golden_root, candidate_root, output_root, drawing=None):
    ...
    manifests = select_manifests(manifests, drawing)
...
def main():
    ...
    parser.add_argument(
        "--drawing",
        help="Optional drawing number. When supplied, analyze only this approved golden drawing.",
    )
    args = parser.parse_args()
    payload, workbook_path, json_path = run_analysis(
        args.golden_root,
        args.candidate_root,
        args.output_root,
