...
from config import (
    ...
    YOLO_SYMBOL_ENSEMBLE_ADDITION_CONFIDENCE,
    YOLO_SYMBOL_ENSEMBLE_ADDITION_CLASSES,
    YOLO_SYMBOL_ENSEMBLE_ADDITION_PATH,
    YOLO_SYMBOL_ENSEMBLE_CONTAINMENT_ENABLED,
    ...
    YOLO_SYMBOL_RESCUE_CLASSES,
    YOLO_SYMBOL_RESCUE_CONFIDENCE,
    YOLO_SYMBOL_RESCUE_PATH,
    YOLO_SYMBOL_TILE_EDGE_MARGIN,
    YOLO_SYMBOL_TILE_OVERLAP,
    YOLO_SYMBOL_TILE_SIZE,
    YOLO_SYMBOL_TILING_ENABLED,
...
_yolo_symbol_rescue_model = None
_yolo_symbol_rescue_available = None
_paddle_ocr = None
...
def get_yolo_symbol_rescue_model():
    """Load an optional proposal-only model used behind strict OCR gates."""
    global _yolo_symbol_rescue_model
    global _yolo_symbol_rescue_available

    if not YOLO_SYMBOL_RESCUE_PATH or YOLO_SYMBOL_RESCUE_CONFIDENCE <= 0:
        return None
    if _yolo_symbol_rescue_available is False:
        return None
    if _yolo_symbol_rescue_model is not None:
        return _yolo_symbol_rescue_model
    model_path = Path(YOLO_SYMBOL_RESCUE_PATH).expanduser()
    if not model_path.is_file():
        _yolo_symbol_rescue_available = False
        if YOLO_SYMBOL_ENSEMBLE_REQUIRED:
            raise FileNotFoundError(f"Required YOLO rescue model was not found: {model_path}")
        logging.warning("YOLO rescue model was not found: %s", model_path)
        return None
    try:
        from ultralytics import YOLO

        _yolo_symbol_rescue_model = YOLO(str(model_path))
        _yolo_symbol_rescue_available = True
        logging.info("Loaded YOLO proposal-only rescue model: %s", model_path)
        return _yolo_symbol_rescue_model
    except Exception as exc:
        _yolo_symbol_rescue_available = False
        if YOLO_SYMBOL_ENSEMBLE_REQUIRED:
            raise RuntimeError(f"Required YOLO rescue model could not be loaded: {model_path}") from exc
        logging.warning("Could not load YOLO rescue model: %s", exc)
        return None
...
def _tile_starts(length, tile_size, overlap):
    ...
    if stride <= 0:
        raise ValueError("YOLO symbol tile overlap must be smaller than tile size")
...
def build_yolo_symbol_tiles(image_shape, tile_size=None, overlap=None):
    """Build overlapping x/y/width/height tiles covering the entire page."""
    image_height, image_width = image_shape[:2]
    resolved_size = YOLO_SYMBOL_TILE_SIZE if tile_size is None else int(tile_size)
    resolved_overlap = (
        YOLO_SYMBOL_TILE_OVERLAP if overlap is None else int(overlap)
    )
    if resolved_size <= 0:
        raise ValueError("YOLO symbol tile size must be positive")
    if resolved_overlap < 0 or resolved_overlap >= resolved_size:
        raise ValueError(
            "YOLO symbol tile overlap must be non-negative and smaller than tile size"
        )
    tile_width = min(image_width, resolved_size)
    tile_height = min(image_height, resolved_size)
    return [
        (x, y, tile_width, tile_height)
        for y in _tile_starts(image_height, tile_height, min(resolved_overlap, tile_height - 1))
        for x in _tile_starts(image_width, tile_width, min(resolved_overlap, tile_width - 1))
    ]


def _translate_tile_detection(
    detection,
    tile,
    image_shape,
    edge_margin=None,
):
    """Map one tile detection to page coordinates, rejecting cut edge boxes."""
    tile_x, tile_y, tile_width, tile_height = tile
    image_height, image_width = image_shape[:2]
    margin = (
        YOLO_SYMBOL_TILE_EDGE_MARGIN
        if edge_margin is None
        else max(0, int(edge_margin))
    )
    local_x = int(detection.get("X", 0))
    local_y = int(detection.get("Y", 0))
    width = int(detection.get("Width", 0))
    height = int(detection.get("Height", 0))
    local_x2 = local_x + width
    local_y2 = local_y + height

    # A box touching an internal crop boundary is probably partial. The
    # overlapping neighbouring tile will contain the complete object.
    if tile_x > 0 and local_x <= margin:
        return None
    if tile_y > 0 and local_y <= margin:
        return None
    if tile_x + tile_width < image_width and local_x2 >= tile_width - margin:
        return None
    if tile_y + tile_height < image_height and local_y2 >= tile_height - margin:
        return None

    page_x1, page_y1, page_x2, page_y2 = clamp_crop_box(
        tile_x + local_x,
        tile_y + local_y,
        tile_x + local_x2,
        tile_y + local_y2,
    ...
    translated.update(
        {
            "X": page_x1,
            "Y": page_y1,
            "Width": page_x2 - page_x1,
            "Height": page_y2 - page_y1,
            "Box": f"{page_x1},{page_y1},{page_x2},{page_y2}",
        }
    )
    return translated


def _detection_overlap(first, second):
    ax1, ay1 = int(first["X"]), int(first["Y"])
    ax2 = ax1 + int(first["Width"])
    ay2 = ay1 + int(first["Height"])
    bx1, by1 = int(second["X"]), int(second["Y"])
    bx2 = bx1 + int(second["Width"])
    by2 = by1 + int(second["Height"])
    ...
    return (
        ...
        intersection / max(1, min(area_a, area_b)),
    )


def deduplicate_tiled_symbol_detections(
    detections,
    iou_threshold=0.45,
    containment_threshold=0.70,
):
    """Remove same-class duplicates created by overlapping inference tiles."""
    kept = []
    ordered = sorted(
        detections or [],
    ...
    for detection in ordered:
        ...
        for existing in kept:
            if detection.get("Symbol Class") != existing.get("Symbol Class"):
                continue
            iou, containment = _detection_overlap(detection, existing)
...
def _predict_symbols_with_tiling(
    model,
    image,
    confidence,
    image_size,
    ...
    image_height, image_width = image.shape[:2]
    if (
        not YOLO_SYMBOL_TILING_ENABLED
        or (image_width <= YOLO_SYMBOL_TILE_SIZE and image_height <= YOLO_SYMBOL_TILE_SIZE)
    ):
        return _predict_symbols_with_model(
            model,
            image,
            confidence,
            image_size,
    ...
    tiles = build_yolo_symbol_tiles(image.shape)
    page_detections = []
    for tile in tiles:
        tile_x, tile_y, tile_width, tile_height = tile
        crop = image[tile_y : tile_y + tile_height, tile_x : tile_x + tile_width]
        tile_detections = _predict_symbols_with_model(
            model,
            crop,
            confidence,
            image_size,
        ...
        for detection in tile_detections:
            translated = _translate_tile_detection(
                detection,
                tile,
                image.shape,
            )
            if translated is not None:
                page_detections.append(translated)

    deduplicated = deduplicate_tiled_symbol_detections(page_detections)
    logging.info(
        "%s tiled inference returned %d boxes from %d tiles; %d remained after deduplication.",
        detector_name,
        len(page_detections),
        len(tiles),
        len(deduplicated),
...
def detect_symbols_with_yolo(image, confidence=None, image_size=1280):
    ...
    baseline = _predict_symbols_with_tiling(
        model,
        image,
        conf,
        image_size,
    ...
    addition = _predict_symbols_with_tiling(
        addition_model,
        image,
        YOLO_SYMBOL_ENSEMBLE_ADDITION_CONFIDENCE,
        image_size,
    ...
    merged, suppressed = baseline_priority_merge(
        ...
        containment_threshold=(
            YOLO_SYMBOL_ENSEMBLE_CONTAINMENT_THRESHOLD
            if YOLO_SYMBOL_ENSEMBLE_CONTAINMENT_ENABLED
            else None
        ),
    ...
    detections = [item["detection"] for item in merged]
    rescue_model = get_yolo_symbol_rescue_model()
    if rescue_model is None:
        return detections
    rescue_classes = {
        normalize_roboflow_class_name(item) for item in YOLO_SYMBOL_RESCUE_CLASSES
    }
    rescue = _predict_symbols_with_tiling(
        rescue_model,
        image,
        YOLO_SYMBOL_RESCUE_CONFIDENCE,
        image_size,
        "local_yolo_ensemble_rescue",
    )
    rescue = [
        item for item in rescue
        if not rescue_classes or item.get("Symbol Class") in rescue_classes
    ]
    combined = deduplicate_tiled_symbol_detections(detections + rescue)
    logging.info(
        "YOLO proposal-only rescue added %d candidate boxes; %d total remained.",
        len(rescue), len(combined),
    )
    return combined
...
def build_position_crop_candidates(image):
    image_height, image_width = image.shape[:2]
    specs = [
        # Include enough of the left side to preserve the complete part name.
        # The old 65% boundary clipped leading words on wide drawings.
        ("fixed_bottom_right_small", 0.58, 0.75, 1.00, 1.00),
