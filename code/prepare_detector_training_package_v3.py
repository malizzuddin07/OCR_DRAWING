"""Build an isolated, balanced detector candidate-v3 training package.

The approved detector_dataset_v4 annotations are never edited. This builder:
...
# The test drawings come from the six drawings added after candidate v1.
# This makes the v1-versus-v3 comparison genuinely independent.
DRAWING_SPLITS = {
    "train": [
...
# These are effective training-instance targets, not new annotations.
# Only already approved training tiles are repeated.
RARE_CLASS_TARGETS = {
    "diameter": 20,
    "chamfer_callout": 20,
}
...
def split_lookup():
    ...
    if len(lookup) != 11:
        raise ValueError(f"Expected 11 unique split drawings, found {len(lookup)}")
    return lookup
...
def read_rows(source: Path):
    ...
    if len(rows) != 639:
        raise ValueError(f"Expected 639 manifest rows, found {len(rows)}")
    return rows
...
def copy_approved_tiles(source: Path, output: Path, lookup, rows):
    ...
    if sum(split_objects.values()) != 639:
        raise ValueError("Copied approved-object total is not 639")
...
def oversample_rare_training_tiles(output: Path, training_tiles, original_counts):
    ...
    for class_name, target in RARE_CLASS_TARGETS.items():
        ...
        while effective[class_name] < target:
            ...
            suffix = f"__v3repeat_{duplicate_number:02d}"
            image_destination = (
                output
                / "images"
                / "train"
...
def prepare(args):
    ...
    training_config = """# Candidate v3: conservative engineering-drawing fine-tuning.
...
"""
    ...
    package_manifest = {
        ...
        "test_drawings_unseen_by_candidate_v1_training": True,
        ...
        "required_gate": (
            "Candidate v3 must beat candidate v1 on the same independent detector "
    ...
    (args.output / "README.md").write_text(
        "# Detector Candidate Training Package v3\n\n"
        "This is an isolated candidate package. It uses 639 unchanged, approved "
