"""Validate or train the isolated detector candidate V7."""
...
DEFAULT_PACKAGE = PROJECT_ROOT / "golden_tests" / "detector_training_package_v7"
CLASS_NAMES = [
    "dimension",
    "diameter",
    "radius",
...
def validate_package(package: Path):
    ...
    if not manifest_path.is_file() or not data_path.is_file():
        raise FileNotFoundError("Training package V7 is incomplete")
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if manifest.get("schema_version") != 7:
        raise ValueError("Training package schema is not V7")
    ...
    if manifest.get("hard_repeat_tile_count") != 10:
        raise ValueError("V7 must repeat each of the 10 hard tiles once")
    if manifest.get("rehearsal_drawing_count") != 9:
        raise ValueError("Rehearsal must cover all 9 training drawings")
    if manifest.get("rehearsal_unique_tile_count", 0) < 9:
        raise ValueError("V7 rehearsal tile count is unexpectedly low")
    if manifest.get("c3010_used_for_training"):
        raise ValueError("C3010 must remain untouched for the reserved gate")
    if not manifest.get("test_drawings_reserved_from_candidate_v7_training"):
        raise ValueError("V7 test drawings are not marked as reserved")
...
def main():
    ...
    plan = {
        ...
        "hard_repeat_tiles": manifest["hard_repeat_tile_count"],
        "rehearsal_tiles": manifest["rehearsal_unique_tile_count"],
        "rehearsal_drawings": manifest["rehearsal_drawing_count"],
    ...
    run_root = PROJECT_ROOT / "runs" / "candidate_detector" / "golden_detector_v7"
    if args.resume:
