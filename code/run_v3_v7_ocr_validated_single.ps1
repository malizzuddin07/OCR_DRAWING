param(
    ...
    [string]$DrawingNumber = "W3-C111265901-03"
)
...
$root = Join-Path $project "golden_tests\runs\v3_v7_ocr_validated_single_${safeDrawingNumber}_$RunId"
...
# Protected candidate configuration.  Production remains unchanged.
# V7 produces the approved tiny dimension `1` at confidence 0.168.  The
# detector threshold is lowered only to 0.15, and every text detection must
# still pass Paddle recognition and engineering-value parsing before it can
# enter the report.
