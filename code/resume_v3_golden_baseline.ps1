param(
    [Parameter(Mandatory = $true)]
    [string]$RunId
)

$ErrorActionPreference = "Stop"
$project = "C:\Users\izzuddin\Desktop\OCR DRAWING"
$python = "C:\Users\izzuddin\python311\python.exe"
$v3 = Join-Path $project "incoming_models\candidate_yolov8s_golden_v3_20260727\golden_detector_v3\weights\best.pt"
$root = Join-Path $project "golden_tests\runs\v3_only_baseline_$RunId"
$output = Join-Path $root "run_1"
$report = Join-Path $root "gate_report"
$log = Join-Path $root "suite_stdout.log"
$status = Join-Path $root "runner_status.txt"

foreach ($required in @($python, $v3, $root, (Join-Path $output "suite_run.json"))) {
    if (-not (Test-Path -LiteralPath $required)) {
        throw "Required path was not found: $required"
    }
}

Set-Content -LiteralPath $status -Value "RESUMING" -Encoding utf8

$env:ROBOFLOW_ENABLED = "0"
$env:YOLO_SYMBOL_MODEL_PATH = $v3
$env:YOLO_SYMBOL_ENSEMBLE_REQUIRED = "0"
$env:YOLO_SYMBOL_CONFIDENCE = "0.60"
$env:YOLO_SYMBOL_TILING_ENABLED = "1"
$env:YOLO_SYMBOL_TILE_SIZE = "1280"
$env:YOLO_SYMBOL_TILE_OVERLAP = "320"
Remove-Item Env:YOLO_SYMBOL_ENSEMBLE_ADDITION_PATH -ErrorAction SilentlyContinue
Remove-Item Env:YOLO_SYMBOL_ENSEMBLE_ADDITION_CLASSES -ErrorAction SilentlyContinue
Remove-Item Env:YOLO_SYMBOL_ENSEMBLE_PREFERRED_CLASSES -ErrorAction SilentlyContinue

Set-Location -LiteralPath $project
$ErrorActionPreference = "Continue"
& $python "code\run_golden_suite.py" `
    --output-root $output `
    --report-dir $report `
    --candidate-id "v3-only-current-code" `
    --resume 2>&1 |
    Tee-Object -FilePath $log -Append
$exitCode = $LASTEXITCODE
$ErrorActionPreference = "Stop"

$suitePath = Join-Path $output "suite_run.json"
if (Test-Path -LiteralPath $suitePath) {
    $suite = Get-Content -LiteralPath $suitePath -Raw | ConvertFrom-Json
    if ($suite.status -in @("passed", "rejected")) {
        Set-Content -LiteralPath $status -Value "COMPLETED ($($suite.status))" -Encoding utf8
        exit 0
    }
}

Set-Content -LiteralPath $status -Value "FAILED_RESUME (exit $exitCode)" -Encoding utf8
exit $exitCode
