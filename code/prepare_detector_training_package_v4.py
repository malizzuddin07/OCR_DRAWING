"""Build an isolated, balanced detector candidate-v4 training package.

The approved detector_dataset_v5 annotations are never edited. This builder:
...
# The newest Drawing 6 stays in test and is never used for candidate-v4 training.
DRAWING_SPLITS = {
    "train": [
        "C3020-060-201F-02",
...
def validate_source(source: Path):
    ...
    if manifest.get("annotation_count") != 965 or manifest.get("drawing_count") != 17:
        raise ValueError("Dataset v5 must contain exactly 965 labels from 17 drawings")
...
def oversample_rare_training_tiles(output: Path, training_tiles, original_counts):
    ...
    for class_name, target in RARE_CLASS_TARGETS.items():
        ...
        while effective[class_name] < target:
            ...
            suffix = f"__v4repeat_{duplicate_number:02d}"
            image_destination = (
                output
                / "images"
                / "train"
...
def prepare(args):
    ...
    training_config = """# Candidate v4: conservative engineering-drawing fine-tuning.
...
"""
    ...
    package_manifest = {
        ...
        "test_drawings_reserved_from_candidate_v4_training": True,
        ...
        "required_gate": (
            "Candidate v4 must beat candidate v3 on the same independent detector "
    ...
    (args.output / "README.md").write_text(
        "# Detector Candidate Training Package v4\n\n"
