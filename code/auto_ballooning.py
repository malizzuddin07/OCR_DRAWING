...
def recover_split_below_radius(text):
    """Keep the decimal value OCR actually read in a split BELOW callout.

    A previous rescue rule inserted an extra zero, turning a visible ``R0.2``
    into ``R0.02``.  Only join the split OCR tokens here; do not manufacture an
    additional decimal digit.
    """
    joined = normalise_text(text).upper().replace("×", "X")
    split = re.search(
    ...
    if not split:
        return ""
    return re.sub(r"\s+", "", f"{split.group(1)}{split.group(2)}")
    ...
    for rotation in rotations:
        ...
        if symbol_class == "dimension_text":
            ...
            if not value and has_nearby_incomplete_radius(symbol, existing_rows):
                value = recover_split_below_radius(joined)
            if value:
...
def are_duplicate_characteristics(row, existing):
    ...
    if not row_value or not existing_value:
        return False
    # Two specialised detectors can assign different symbols to the exact same
    # crop (for example radius ``R9.5`` and diameter ``Ø9.5``). If the
    # nominal and geometry are effectively identical, they are competing
    # classifications of one printed characteristic, not two characteristics.
    compatible_symbol_types = {
        "plain_dimension",
        "diameter",
        "radius",
        "chamfer",
    ...
    same_source_geometry = box_iou(row, existing) >= 0.80 or box_containment(row, existing) >= 0.90
    if (
        row_value == existing_value
        and row_type in compatible_symbol_types
        and existing_type in compatible_symbol_types
        and same_source_geometry
    ):
        return True
