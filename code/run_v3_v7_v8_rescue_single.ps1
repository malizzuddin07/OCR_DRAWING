...
$root = Join-Path $project "golden_tests\runs\v3_v7_v8_rescue_single_${safeDrawingNumber}_$RunId"
$job = Join-Path $root "job"
$log = Join-Path $root "console.log"

foreach ($required in @($python, $pdf, $v3, $v7, $v8)) {
...
# V3 is the primary detector, V7 adds all normal classes, and V8 may only
# propose ultra-low-confidence dimensions. V8 proposals need high-confidence
# crop OCR before they can enter the report.
$env:ROBOFLOW_ENABLED = "1"
...
if ($pythonExitCode -ne 0) {
    throw "Protected V3+V7+V8 OCR runner failed with exit code $pythonExitCode. See: $log"
}
exit 0
