"""Build detector dataset v5 from the locked v4 dataset plus six approvals.
...
NEW_DRAWING_SPLITS = {
    "C3020-060-201F-02": "train",
    "W3-C090489701-1A": "train",
    "W3-C111262901-0B": "train",
    "W3-C081236901-00": "val",
    "W3-C111265401-00": "val",
    "W3-C111266101-01": "test",
}
...
def build(args):
    ...
    if int(base_manifest["annotation_count"]) != 639:
        raise ValueError("The locked v4 base no longer contains exactly 639 labels")
    if int(base_manifest["drawing_count"]) != 11:
        raise ValueError("The locked v4 base no longer contains exactly 11 drawings")
    ...
    approved_new_total = sum(
        item["approved_label_count"] for item in approval_summaries
    )
    if approved_new_total != 326:
        raise ValueError(
            f"Expected 326 newly approved labels, found {approved_new_total}"
        )
    expected_total = 639 + approved_new_total
    if expected_total != 965 or len(records) != expected_total:
        raise ValueError(f"Expected 965 combined labels, built {len(records)}")
    ...
    manifest = {
        "schema_version": 1,
        "status": "combined_17_drawing_dataset_ready_for_approval_not_training",
        "training_allowed": False,
        "production_model_change_authorized": False,
        "purpose": "Combine locked detector dataset v4 with six additional Stage 2 human-approved drawings",
        "base_dataset": str(args.base_dataset),
        "base_dataset_manifest_sha256": sha256(base_manifest_path),
        "base_annotation_count": 639,
        "new_annotation_count": 326,
        ...
        "required_next_step": "Verify the 17-preview provenance and approve dataset v5 before candidate training package preparation",
    }
    ...
    report.extend(
        [
            ...
            "",
            "The 11 base previews retain their v4 approval and the six new previews retain their individual Stage 2 approvals.",
            "Only after approval hashes and all 17 previews are verified may a new candidate training package be prepared.",
        ]
    )
